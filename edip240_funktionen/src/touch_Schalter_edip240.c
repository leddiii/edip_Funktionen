...
|*Description:
|*
|*Zeichnet einen Touch-Schalter im Bereich x1,y1,x2,y2. Der downcode und der upcode wird in den Sendepuffer übertragen,wenn der Schalter betätigt wird.
...
|*18 Rahmen, finden Sie in der Bedienungsanleitung für das edip240 auf der Seite 10.
|*
|*Übergabeparameter:
|*
|*dev_adr: 	Adressierungsbyte des Slaves.
|*x1:		ertser Punkt auf der X-Achse des Displays für den Button					(0-239)
|*y1:		erster Punkt auf der Y-Achse des Displays für den Schalter					(0-127)
|*x2:		zweiter Punkt auf der X-Achse des Displays für den Button					(0-239)
|*y2:		zweiter Punkt auf der Y-Achse des Displays für den Schalter					(0-127)
...
|*rahmen:	Art des gewünschten Rahmens siehe Bedienungsanleitung Seite 10				(1-18)
|*
...
uint8_t edip240_touch_schalter(uint8_t dev_adr, uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint8_t downcode, uint8_t upcode, char *Text, uint8_t rahmen)
{
    ...
    Sendbuf[9] = downcode;
    Sendbuf[10] = upcode;
    Sendbuf[11] = 0x43; // C für text begin
...
|*
|*Schalter1=edip240_SchalterStellung(0xDE) Diese funktion sollte so im Interrupt aufgerufen werden. Da die funktion einen Wert zurück gibt,
